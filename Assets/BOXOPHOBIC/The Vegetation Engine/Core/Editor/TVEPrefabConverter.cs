...
namespace TheVegetationEngine
{
    public class TVEPrefabConverter : EditorWindow
    {
        ...
        void OnEnable()
        {
            ...
            collectInCustomFolder = Convert.ToBoolean(SettingsUtils.LoadSettingsData(userFolder + "Converter Folder.asset", "False"));
        ...
        void DrawConversionSettings()
        {
            ...
            if (presetIndex != 0)
            {
                ...
                if (showAdvancedSettings)
                {
                    ...
                    {
                        EditorGUILayout.HelpBox("When Collect Converted Data is enabled, all prefabs and the associated assets are moved or copied to a new folder of your choice for better project organization! The following settings are saved for futhur conversions!", MessageType.Info, true);
                    }
                    ...
                    {
                        GUILayout.BeginHorizontal();
                        GUILayout.Label(new GUIContent("Collect Original Textures", "Copy all original textures as new assets to the Assets Data/Original Data folder."), GUILayout.Width(GUI_HALF_EDITOR_WIDTH - 5));
                        collectOriginalTextures = EditorGUILayout.Toggle(collectOriginalTextures);
                        GUILayout.EndHorizontal();

                        GUILayout.BeginHorizontal();
                        GUILayout.Label(new GUIContent("Collect Prefabs As New", "Save a new prefab instead of replacing the old one."), GUILayout.Width(GUI_HALF_EDITOR_WIDTH - 5));
